...
namespace Utilla.Patches
{
    ...
    internal static class GorillaComputerPatches
    {
        ...
        public static bool UpdateModeTextPatch(GorillaComputer __instance)
        {
            ...
            currentGameModeText.Value = $"{currentMode}\n{(!(gamemode is null) ? gamemode.DisplayName.ToUpper() : GorillaScoreBoard.error)}";

            return false;
        }

        [HarmonyPatch(nameof(GorillaComputer.InitializeGameMode), argumentTypes: new Type[0]), HarmonyTranspiler]
        ...
        [HarmonyPatch(nameof(GorillaComputer.InitializeGameMode), argumentTypes: new Type[0]), HarmonyPrefix]
